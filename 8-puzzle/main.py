...
def A_Star_window():
    ...
    eqlid_type = Button((SCREEN_WIDTH//2)-(BUTTON_WIDTH//2), (SCREEN_HEIGHT//2)-(BUTTON_HEIGHT//2)-100 + 50 + BUTTON_HEIGHT + SIDES_PADDING, BUTTON_WIDTH,BUTTON_HEIGHT,BUTTONS_COLOR," Eucledian Type",TEXT_COLOR,FONT_SIZE2+5)
...
if start_player:
    ...
    start = time.time()
    path, moves, depth, no_exploBUTTONS_COLOR = agent.work()
    end = time.time()
    ...
    total_no_moves_text = Button(SIDES_PADDING+(2*witdth)+(2*INBTWN_SPACE), 10, witdth, BUTTON_HEIGHT-60, BUTTONS_COLOR," # of explored: " + str(no_exploBUTTONS_COLOR),TEXT_COLOR,FONT_SIZE2-10)
